...
local remote = ReplicatedStorage:WaitForChild("563377f9295bd02c88f984f6ea3de042")
local MULTIPLIER = 100

local launchEnabled = false
local launchThread = nil

local function startLaunch()
    if launchThread then return end
    launchEnabled = true
    launchThread = task.spawn(function()
            ...
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= Players.LocalPlayer then
                    pcall(function()
                        remote:FireServer(
                            player,
                            Vector3.new(math.random()*2-1, 0, math.random()*2-1),
                            true,
                            MULTIPLIER
                        )
                    end)
